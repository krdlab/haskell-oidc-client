...
import qualified Web.OIDC.Client.Discovery.Provider
                                               as P
...
import           Web.OIDC.Client.Types          ( Code
                                                , Nonce
                                                , OpenIdException(..)
                                                , Parameters
                                                , Scope
...
prepareAuthenticationRequestUrl store oidc scope params = do
  ...
  getAuthenticationRequestUrl oidc scope (Just state)
    $  params
    <> [("nonce", Just nonce')]
...
getValidTokens store oidc mgr stateFromIdP code = do
  ...
  if state == Just stateFromIdP
    then do
      when (isNothing savedNonce) . throwM $ ValidationException
        "Nonce is not saved!"
...
getAuthenticationRequestUrl oidc scope state params = do
 ...
 where
  ...
  requireds =
    ...
    , ("scope", Just . B.pack . unwords . nub . fmap unpack $ openId : scope)
    ]
  state' = case state of
    Just _  -> [("state", state)]
    Nothing -> []
...
requestTokens
  :: FromJSON a => OIDC -> Maybe Nonce -> Code -> Manager -> IO (Tokens a)
...
validate
  :: FromJSON a => OIDC -> Maybe Nonce -> I.TokensResponse -> IO (Tokens a)
validate oidc savedNonce tres = do
  let jwt' = I.idToken tres
  claims' <- validateIdToken oidc jwt'
  now     <- getCurrentIntDate
  validateClaims (P.issuer . P.configuration . oidcProvider $ oidc)
...
validateClaims
  :: Text -> Text -> Jwt.IntDate -> Maybe Nonce -> IdTokenClaims a -> IO ()
validateClaims issuer' clientId' now savedNonce claims' = do
  ...
  unless (iss' == issuer')
    ...
    $  "issuer from token \""
    <> iss'
    <> "\" is different than expected issuer \""
    <> issuer'
    <> "\""
  ...
  unless (clientId' `elem` aud')
    ...
    $  "our client \""
    <> clientId'
    <> "\" isn't contained in the token's audience "
    <> (pack . show) aud'

  unless (now < exp claims') . throwM $ ValidationException
    "received token has expired"

  unless (nonce claims' == savedNonce) . throwM $ ValidationException
    "Inconsistent nonce"
